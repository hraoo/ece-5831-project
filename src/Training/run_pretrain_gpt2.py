...
def mCollateFn(batch):
	...
	for f in features:
		...
		# print('f0 len', len(f[0]))
		for i in range(len(f[0])):
			# print(f[0][i])
			sequence = f[0][i] + [mask_token]*(max_len-len(f[0][i]))
			# print('s', sequence)
			# print('len', (max_len-len(f[0][i])))
		...
		if len(batch_input_ids)>1:
			if len(batch_input_ids[0])!=len(batch_input_ids[1]):
			
				print('Error, but can continue')
				batch_input_ids[1]=batch_input_ids[0]
				batch_input_mask[1] = batch_input_mask[0] 
				batch_input_labels[1] = batch_input_labels[0]
				batch_label_ids[1] = batch_label_ids[0]
