...
skip_words.add('\'s')
skip_words.add('.')
skip_words.add('PersonX')
skip_words.add("PersonX's")
skip_words.add('personX')
skip_words.add('PersonY')
skip_words.add("PersonY's")
skip_words.add('personY')
skip_words.add("'d")
skip_words.add(',')
skip_words.add('___')
...
import argparse
import sys
sys.path.insert(1, r'C:\Users\neelk\Paper\trialS\NK-AKG-main\src')
...
class ATOMICProcessor(object):
    def __init__(self, args):
        ...
        'oEffect': '. Others then',
        'isBefore': '. What happened before was',
        'isAfter': '. What happened after was',
        'AtLocation': ' can be found in',
        'CapableOf': '. It is capable of',
        'Causes': '. That caused',
        'Desires': '. Resulting in desire of',
        'HasProperty': '. It also contains',
        'HasSubEvent': '. But first need to',
        'HinderedBy': '. It can not happen because',
        'MadeUpOf': '. It is made up of',
        'NotDesires': '. It does not want to',
        'ObjectUse': '. It is used to',
        'xReason': '. PersonX did that because',
        'isFilledBy': ''
        }
        ...
        with open(filename, "r") as f:
            ...
            for row in tqdm(csvreader):
                if row[0] != previous:
                    ...
                    prefix = [w for w in row[0].split() if w not in skip_words]
                ...
                #     for ending in attr:
                ending = ending.lower()
                ending = self.fix_templates(self.mapping[attr], text_standardize(ftfy.fix_text(ending)))
                if '_' in ending:
                    tok = self.find_underscore_length(ending)
                    ending = ending.replace(tok, "___")
                if ending != 'none.' and len(ending) > 0 and ending not in cache[attr]:
                    self.tail_keywords[(ending, attr)] |= set(prefix)
                    cache[attr].append(ending)
                if '_' in ending:
                    print('here')
